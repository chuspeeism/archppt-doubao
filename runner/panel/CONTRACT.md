...
{"ts":"<ISO>","type":"phase","phase":"received","title":"AI Agent 产品架构","nodes":14,"edges":9,"specPath":"/abs/path"}
{"ts":"<ISO>","type":"phase","phase":"layout","total":27,"byKind":{"title":1,"subtitle":1,"container":3,"node":14,"edge":9,"label":1}}
{"ts":"<ISO>","type":"phase","phase":"auth"}
{"ts":"<ISO>","type":"step","i":1,"total":27,"kind":"node","id":"gateway","label":"API 网关","status":"start"}
{"ts":"<ISO>","type":"step","i":1,"total":27,"kind":"node","id":"gateway","label":"API 网关","status":"done","ms":163}
{"ts":"<ISO>","type":"phase","phase":"saving"}
{"ts":"<ISO>","type":"phase","phase":"done","file":"/Users/sunyi/Desktop/架构图-20260912-1030.pptx","shapes":27,"elapsedMs":9800,"animation":true}
...
| `step` | 每个图元画之前、画完各一条 | 驱动器（`project/drivers/mac-powerpoint.mjs`） |
...
- `received`：收到并且**校验通过**了一份 spec。带 `title` / `nodes` / `edges` / `specPath`。
...
- `auth`：**授权预检**——真画之前先用一条最小指令探 PowerPoint 有没有人应答。
  macOS 第一次让豆包控制 PowerPoint 会弹一次「豆包工作想要控制 Microsoft PowerPoint」，
  这个框被别的系统弹窗挡住时**不会出现**，指令就干等到超时（-1712）。面板在这一阶段
  显示「正在确认 PowerPoint 授权」+「若系统弹出授权框，请点「允许」」，
  不许再停在「布局完成」装作什么都没发生。没有额外字段。
...
- `done`：完事。`file` 是 pptx 的绝对路径，`shapes` 是幻灯片上真实的形状数，
  `elapsedMs` 是驱动器从起到落的耗时，`animation` 是「进入动画有没有加上」
  （加不上不算失败，见 README 的「尽力而为」一节）。
...
| 退出码 | 最后一行 | 什么意思 |
|---|---|---|
| 0 | `已保存：<pptx 绝对路径>` | 成功（`--dry-run` 是 `DRY-OK steps=n shapes=m`） |
| 2 | `失败：spec 校验失败：…` / `失败：输出路径 … 不在你的用户目录下…` | spec 读不了、没过校验，或者 `--out` 落在 PowerPoint 沙盒写不进去的地方 |
| **3** | `失败：需要授权 —— <判词>` | **PowerPoint 没应答，要人去点那个授权框**。预检没过（一步都没画）的 -1712 / -1743，以及任何阶段的 -1743 |
...
3 是去点「豆包工作想要控制 Microsoft PowerPoint」那个框；4 是去装桌面版 PowerPoint
...
```jsonc
{
  ...
  "done":    [ { "i": 1, "kind": "title", "id": "title", "label": "…", "ms": 120 } ],
