...
description: 把系统描述变成架构图，并驱动本机 PowerPoint 逐个图元画出来、存成可编辑的 pptx。用户说「帮我画一个……的架构图」「画一张架构图」「架构图 PPT」「画架构图到 PowerPoint」「架构图逐步绘制」「多张架构图放一份 PPT」，或要把一套系统/链路/平台画成架构图时使用；画完之后用户说「换个皮肤」「换个风格」「换成讲义风」「换个配色重画一份」，要给同一套图换一款皮肤的，也走这个技能。
---
...
**1.5 先问用户要哪款皮肤，问完停下等他回答。** 结构想清楚了、**动手写 JSON 之前**，
先用**一条消息、一个问题**把四款皮肤摆出来让他四选一：
...
末尾加一句「不选就用默认 `stratum`」。问句**照抄下面这个模板**（每次都一样，用户才好比较）：

```text
这套架构图想用哪款皮肤？回一个序号或名字就行：
1. stratum（默认）—— 技术文档、方案评审，中性偏冷的浅色
2. handbook —— 课程讲义、培训材料，分组一眼看得出
3. journal —— 论文、白皮书，正文区留得最干净
4. source —— README、设计评审，偏工程风
不选就用默认 stratum。
```

他回序号就按上面的顺序对应到 id。**问完就停，等用户回答。** 同一条回复里不许接着贴 JSON、
不许替他选一款就往下走、不许一边问一边开画。

他回答之后，把选中的 id 写进 JSON 顶层的 `skin`（多页 deck 就写进**每一页**，四款里挑一款、
页页相同）。他回「默认 / 随便 / 都行 / 你定」就算选了 `stratum`，**照样把 `skin` 写进去**。

**三种情况不问，直接往下走：**

- ① **用户在描述里已经点名了风格或皮肤**——直接说了这四个 id 之一，或者说法明显对应其中一款
  （「培训材料风格」→ `handbook`，「像论文/白皮书那样」→ `journal`，「工程风、README 那种」→ `source`）。
  按它写 `skin` 就行。
- ② **用户说「重试」**（见第 6 步）——JSON 已经落盘，不重新生成，也不再问皮肤。
- ③ **用户在换皮肤**（见第 7 步）——他这句话里已经说了要哪款。

用户点的风格**不在这四款里**（比如「深色」「暗黑」「科技感」「手绘风」）：告诉他这一版只有上面
这四款，请他从中挑一款。**不要硬写别的 id**——引擎认不出来会回退成默认皮肤，画出来跟说好的不一样。
...
`--delay 350` 是**大元素**（标题、副标题、容器大框、节点）每画完一个停多久 —— 停顿是给人看的，
让图元一个一个出现。**连线和线上的文字默认快画**（`--quick-delay` 默认 0，不用写）：
它们又小又多，逐个停顿只是让人干等。
...
**7. 画完之后用户要换皮肤时。** 触发的说法：「换成 handbook 皮肤」「换个风格」「换成讲义风」
「试试 journal」「这套换个配色」。

**不要重新生成 JSON、不要再问一遍系统描述、也不要再问一遍皮肤**（第 1.5 步那个问题这一轮不问，
他这句话里已经点名了）。取 `<技能目录>/runner/.state/inbox/` 里**最新的那份 spec**
（按文件名里的时间戳排最后一个），对它执行第 4 步那条命令，只多带一个 `--skin`：

```bash
node "<技能目录>/runner/run.mjs" "<最新 spec 绝对路径>" --skin <皮肤 id> --delay 350
```
...
`--skin` 会把**每一页**的皮肤都覆盖成这一款，spec 文件本身不改、也不用新写一份。

- **结果照第 5 步报**：最后一行 `已保存：…` 原样给用户，多页时上一行的 `共 k 页` 也一并说。
  再补一句：这是**另存出来的新文件，原来那份 pptx 原封不动**，两个配色都留着。
- 用户要的皮肤**不在四款里** → 把四款报给他让他挑一款，**先不执行**。
- 皮肤 id 写错时绘制器会直接停（退出码 2，最后一行 `失败：spec 校验失败：皮肤 "<id>" 不存在（…）`）。
  这属于第 5 步的第 ① 种，是你写错了不是用户写错了：自己换成四款里的正确 id 重跑一次，
  不要把这行报错原文贴给用户。
- 换完皮肤之后用户再说「重试」，仍旧跑**带着 `--skin` 的那条命令**，别把它丢了。
- 提醒他一句：换皮肤是在 PowerPoint 里把整套图**从头重画一遍**，时间跟第一次差不多
  （多页按页数乘），中途不要打断、也不要重复执行。
...
- **每一页都要写 `skin`，而且写成同一款**——就是第 1.5 步用户挑的那一款。同一份 PPT 里
  页与页皮肤不一样会很怪。
...
`skin` 配色皮肤，写第 1.5 步用户挑的那一款，见第四节「皮肤」的四选一；
...
配色皮肤。**只用下面这四个**，写别的引擎认不出来、会回退成默认。

按第二节第 1.5 步**先问用户挑一款，拿到答案就把 id 写进 `skin`**——他说「默认 / 随便」也写，
写 `stratum`。多页 deck 每一页都写同一款。只有「重试」（第 6 步）和「换皮肤」（第 7 步）
这两种不重新生成 JSON 的情况谈不上写不写。
